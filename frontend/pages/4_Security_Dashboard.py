...
from utils import api_get, current_role

st.set_page_config(page_title="Security Dashboard — Shield AI", layout="wide")
...
with st.container(border=True):
    st.markdown(
        "### 🛡 Two-layer security gate (runs before any LLM call)\n\n"
        "**Layer 1 — [Veea Lobster Trap](https://github.com/veeainc/lobstertrap):** "
        "deep prompt inspection proxy running locally. Every uploaded contract is "
        "sent through Lobster Trap's `/v1/chat/completions` endpoint where its "
        "DPI engine checks for prompt injection, credential leaks, data "
        "exfiltration patterns, role impersonation, and policy bypass attempts. "
        "If it returns `verdict: DENY`, the contract is quarantined immediately.\n\n"
        "**Layer 2 — offline pattern detector:** 13 regex patterns + zero-width "
        "unicode detection. Runs regardless of Lobster Trap availability, so the "
        "system degrades gracefully and provides defense in depth. Each "
        "`SecurityEvent` records its `source` so we know which layer caught what."
    )
