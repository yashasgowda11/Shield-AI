...
st.set_page_config(page_title="Knowledge Graph — Shield AI", layout="wide")
...
NODE_VISUAL = {
    "vendor":    {"shape": "square",   "color": "#3b82f6"},  # blue
    "contract":  {"shape": "circle",   "color": "#22c55e"},  # green (overridden by status)
    "risk":      {"shape": "triangle", "color": "#ef4444"},  # red (severity-based)
    "framework": {"shape": "hexagon",  "color": "#a855f7"},  # purple
}
...
def _node_size(meta: dict) -> int:
    """Bigger nodes for hubs (high in/out degree implied by occurrences/n_contracts)."""
    if meta.get("type") == "risk":
        return 20 + min(int(meta.get("occurrences", 1)) * 6, 40)
